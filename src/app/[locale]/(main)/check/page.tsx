...
const BankPaymentButton: React.FC<{
  ...
  >
    ...
    <p className={`font-semibold text-sm ${checked ? 'text-black' : 'text-gray-500'}`}>
      Bank Account
    </p>
  </button>
);
...
const KhaltiButton: React.FC<{ checked: boolean; onClick: () => void }> = ({ checked, onClick }) => (
  <button
    onClick={onClick}
  ...
  >
    <img src="/images/icons/khalti.png" height={20} width={20} alt="khalti_logo" />
    <p className="text-[#800080] font-poppins text-base font-semibold leading-[1.4em] w-fit">Khalti</p>
  </button>
);

const ItemCounter: React.FC<{ productId: string; initial?: number }> = ({ productId, initial = 1 }) => {
    ...
    <div className="flex items-center gap-1">
      ...
      >
        <svg width="6" height="2" viewBox="0 0 6 2" fill="none">
          <path d="M5.08844 0.000187397V1.41619H0.000437528V0.000187397H5.08844Z" fill="#3E3E3E" />
        </svg>
      </button>

      <span className="text-sm font-semibold text-[#0000FF] w-4 text-center">{count}</span>
      ...
      >
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M15.061 12.46H12.793V14.788H11.209V12.46H8.94103V10.996H11.209V8.668H12.793V10.996H15.061V12.46Z" fill="#3E3E3E" />
        </svg>
      </button>
    </div>
  );
...
const UserForm: React.FC<{ onSave: (address: Address) => void; initialData?: Address }> = ({
  onSave,
  initialData,
}) => {
    ...
    <div className="bg-white p-4 rounded-[16px] border border-[#F5F5F6] shadow-[0_4px_4px_rgba(0,0,0,0.25)] flex flex-col gap-3 mx-4 md:mx-0 mt-4">
      <div className="flex justify-between items-start gap-[54px]">
        <div className="flex flex-col gap-2 w-[178px]">
          <h2 className="text-lg leading-[25px] font-semibold text-[#333333]">Your Details</h2>
          <p className="text-sm leading-[20px] font-medium text-[#444]">Sabina Pandit</p>
        ...
        <div className="ml-auto flex flex-col gap-2">
          {address ? (
            ...
            <Button
              variant="primary"
              onClick={onAdd}
              className="rounded-md px-4 py-2 text-sm shadow-sm hover:shadow-md transition-all duration-200"
            >
              Add Address
            </Button>
          )}
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[#444444] font-medium text-[12px] md:text-sm leading-tight truncate">{item.name}</p>
        {item.color ? (
...
const OrderSummarySection: React.FC<{ items: OrderItem[] }> = ({ items }) => {
  ...
  return (
    <div className="bg-white p-4 rounded-[16px] border border-[#F5F5F6] shadow-[0_4px_4px_rgba(0,0,0,0.25)] mx-4 md:mx-0 mt-6">
      <h2 className="text-lg font-semibold text-[#333333] mb-1">Order Summary</h2>
  ...
  />
      ...
      <div className="pt-4 pb-4 border-b border-gray-100">
        <div className="flex justify-between items-center py-1">
          ...
          <span className="text-sm font-medium text-[#444444]">Rs {subtotal.toLocaleString()}</span>
        </div>
        <div className="flex justify-between items-center py-1">
          <span className="text-sm font-medium text-[#777777]">Delivery Charge</span>
          <span className="text-sm font-medium text-[#FF0000]">Rs {deliveryCharge.toLocaleString()}</span>
        </div>
        <div className="flex justify-between items-center py-1">
          <span className="text-sm font-medium text-[#777777]">Service Fee</span>
          <span className="text-sm font-medium text-[#444444]">Rs {serviceFee.toLocaleString()}</span>
        </div>
      </div>
      <div className="flex justify-between items-center pt-4">
        <span className="text-[#222222] font-medium text-base">Total Payable</span>
        <span className="text-[#222222] font-semibold text-lg">Rs {totalPayable.toLocaleString()}</span>
      </div>
    </div>
  );
};
...
const PaymentMethodSection: React.FC<{ selected: string; onSelect: (method: string) => void }> = ({ selected, onSelect }) => (
  <div className="bg-white p-4 rounded-[16px] border border-[#F5F5F6] shadow-[0_4px_4px_rgba(0,0,0,0.25)] mx-4 md:mx-0 mt-6">
    <h2 className="text-lg font-semibold text-[#333333] mb-4">Select Payment Method</h2>
    <div className="flex justify-start gap-3 mb-4">
      <BankPaymentButton checked={selected === "bank"} onClick={() => onSelect("bank")} />
      <KhaltiButton checked={selected === "khalti"} onClick={() => onSelect("khalti")} />
    </div>

    {selected === "bank" && (
      <div className="flex flex-col gap-1 mb-4">
        <label className="text-sm font-medium text-[#444]">Account Number</label>
        <input
          type="text"
...
const CheckoutPage: React.FC = () => {
  ...
  if (!items.length) return <div className="text-center mt-10">Your cart is empty</div>;
  ...
  return (
    <div className="min-h-screen pb-8 overflow-x-hidden">
      ...
      <main className="max-w-md mx-auto relative z-0">
        ...
        <PaymentMethodSection selected={selectedPayment} onSelect={setSelectedPayment} />
      </main>
